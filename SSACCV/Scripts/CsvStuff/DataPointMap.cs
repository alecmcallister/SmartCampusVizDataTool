...
public sealed class DataPointMap : ClassMap<DataPoint>
{
	...
	//	Map(m => m.yyc_x).Name("yyc_x_start").ConvertUsing(row => TryConvertDecimal(row, "yyc_x_start", 0m));
	//	Map(m => m.yyc_y).Name("yyc_y_start").ConvertUsing(row => TryConvertDecimal(row, "yyc_y_start", 0m));
